...
While introducing backpressure support to RxJava, it turned out that all operators that needed to support this feature had to be reimplemented for the \code{request(n)} to be used. Above that, not every operator is able to support backpressure, since they convert to hot streams or it uses another flow control mechanism like time or other streams. Detailed reasons per operator for not implementing backpressure can be found in the RxJava documentation \cite{rx-api}. Other operators, that got reimplemented and supported backpressure, generally became more complex compared to the non-backpressure implementations.

As with Reactive Streams, the \obs coming from cold source is ideal for the reactive pull model of backpressure. Since the source is interactive, methods like \code{request} can be implemented. The RxJava wiki acknowledges the fact that the hot \obs is typically not suitable for the reactive pull model \cite{RxJava-Wiki-HotCold}. In order to keep supporting these streams, RxJava made backpressure optional (one can still implement its own \obs as before using \code{Observable.create}) and also advises other flow control strategies, as discussed in section~\ref{subsec:avoiding-overproduction}, as well as the \code{onBackpressureDrop} and \code{onBackpressureBuffer} operators that were mentioned in section~\ref{subsec:reactive-pull}.




%Given the RS API, we assume that we are in a closed world, according to Erik. The source has to be able to satisfy the requests. This is not possible with a hot stream. RS en Reactive Pull (Rx) are most suitable for cold streams (see \cite{RxJava-Wiki-HotCold} as well)

%We need to define the semantics on the Reactive Streams API, i.e. define it as a monad. Since the values are pushed at you, you need to merge streams. In the list monad you do concatenation, however, this does not work on potential infinite collections like the ones in RS. Therefore merging is the way to go. But how to do merging in a `request' way. Suggested by Erik is dovetailing. The RS website does not say anything about this. No semantics are specified.

%Other problems involve operators like GroupBy. See example Erik op 37:26 => deadlock situation. You need to buffer for this? RS does not say anything about this.

%Time or mouse moves is not possible in RS. Semantics need to be defined!



%Google, just as RxJS has pause/resume. Something for previous chapter... see 43:38
